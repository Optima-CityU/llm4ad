...
def system_prompt_observation(obs_layer: int):
...
def get_observations_prompt(
        _task_description_str, highest_level_obs: tuple[str, ...], num_observations_to_generate: int, obs_layer: int
) -> tuple[dict[str, str], dict[str, str]]:
    sys_prompt = system_prompt_observation(obs_layer)
    if obs_layer == 0:
        user_prompt = get_observation(_task_description_str, num_observations_to_generate)
    else:
        user_prompt = get_combine_observations(_task_description_str, highest_level_obs, num_observations_to_generate)
    conversation = (
...
def get_observation(_task_description_str, num_observations: int) -> str:
...
def get_combine_observations(_task_description_str, obs_combo: tuple[str, ...], num_observations: int) -> str:
    prompt = f"Here is the problem:\n\n{_task_description_str}\n\n"
