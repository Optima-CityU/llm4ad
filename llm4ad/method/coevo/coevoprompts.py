...
class CoEvoPrompt:
    ...
    def get_init_observation_prompt(
        cls, task_info: TaskInfo, coevo_paras: CoEvoParas, highest_level_obs: tuple[str, ...], obs_layer_i: int
    ):
        conv_content = init_prompts.get_observations_prompt(
            task_info, highest_level_obs, coevo_paras.num_observations_to_generate, obs_layer_i
        )
    ...
    def get_init_impl_direct_prompt(
            cls, task_info: TaskInfo, coevo_paras: CoEvoParas, indirect_sol: str
    ):
        conv_content = init_prompts.get_direct_impl_prompts(
            task_info, coevo_paras, indirect_sol
        )

        return conv_content

    @classmethod
    def get_init_impl_indirect_prompt(
            cls, task_info: TaskInfo, coevo_paras: CoEvoParas, use_combo: tuple[str, ...]
    ):
        conv_content = init_prompts.get_indirect_impl_prompts(
            task_info, coevo_paras, use_combo
        )
    ...
    def get_offspring_prompt(
            cls, task_info: TaskInfo, evo_paras: CoEvoParas,
            parents: list, mode: str = "crossover_positive",
            use_summary: bool = False, summarizer=None
    ):
        if use_summary:
    ...
    @classmethod
    def get_single_summarizer_prompt(cls, task_info: TaskInfo, evo_paras: CoEvoParas, previous_result_dict_list, inspiration_pool):
