...
class VolcengineAPI(LLM):
    def __init__(self, host='ark.cn-beijing.volces.com', key=None, model=None,
                 path='/api/v3/chat/completions', timeout=60, **kwargs):
        """Volcengine API
        Args:
        ...
        """
        if key is None:
            raise ValueError('VolcengineAPI requires key.')
        if model is None:
            raise ValueError('VolcengineAPI requires model.')
...
class DoubaoAPI(VolcengineAPI):
    """Alias for VolcengineAPI."""
