...
class TencentCloudAPI(LLM):
    def __init__(self, host='tokenhub.tencentmaas.com', key=None, model=None,
                 path='/v1/chat/completions', timeout=60, **kwargs):
        """Tencent Cloud API
        Args:
        ...
        """
        if key is None:
            raise ValueError('TencentCloudAPI requires key.')
        if model is None:
            raise ValueError('TencentCloudAPI requires model.')
