...
class ZhipuAPI(LLM):
    def __init__(self, host='open.bigmodel.cn', key=None, model=None,
                 path='/api/paas/v4/chat/completions', timeout=60, **kwargs):
        """Zhipu AI API
        Args:
        ...
        """
        if key is None:
            raise ValueError('ZhipuAPI requires key.')
        if model is None:
            raise ValueError('ZhipuAPI requires model.')
