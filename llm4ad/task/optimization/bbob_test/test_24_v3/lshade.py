...
from llm4ad.task.optimization.bbob_initial.jade import JADE  # adaptive differential evolution (JADE)
...
class LSHADE(JADE):
    ...
    def mutate_de(self, x=None, y=None, a=None):
        ...
        for i in range(self.n_individuals):
            ...
            v[i] = x[r[0]] + 0.5*(x[r[1]] - x[r[2]])
        return v, np.array([0.5]*self.n_individuals), self.rng_optimization.choice(self.h, (self.n_individuals,))
    ...
    def crossover_de(self, v=None, x=None, r=None):
        ...
        for i in range(self.n_individuals):
            ...
            tmp = v[i, j_r]
            co = self.rng_optimization.random(self.ndim_problem) > 0.9
            v[i, co] = x[i, co]
            v[i, j_r] = tmp
        return v, np.array([0.9]*self.n_individuals)  # crossover probabilities
    ...
    def iterate(self, x=None, y=None, a=None, args=None):
        x_mu, f_mu, r = self.mutate_de(x.copy(), y.copy(), a.copy())
        x_cr, p_cr = self.crossover_de(x_mu.copy(), x.copy(), r.copy())
