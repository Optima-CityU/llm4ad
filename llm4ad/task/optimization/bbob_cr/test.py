...
def crossover(self, x_mu=None, x=None, r=None):
    ...
    # Quantum coherence time based on generation progress
    coherence_time = np.pi * self._n_generations / (self.max_function_evaluations / self.n_individuals)
    
    # Population density for decoherence rate
    if self.h > 0 and len(self.m_median) > 0:
        density_metric = np.mean(np.abs(self.m_median))
        decoherence_rate = 2.2 + 0.8 * np.exp(-density_metric)
    else:
        decoherence_rate = 2.2
    
    # Entanglement field with exponential decay
    field_strength = np.exp(-np.arange(self.n_individuals) / (self.n_individuals / 3))
    quantum_phases = coherence_time + field_strength * 4 * np.pi
    
    # Sort by field strength for quantum ordering
    quantum_order = np.argsort(-field_strength)
    
    for idx, i in enumerate(quantum_order):
        # Superposition amplitude for crossover probability
        superposition_amp = 0.7 + 0.5 * np.cos(decoherence_rate * quantum_phases[i])
        
        # Fitness-weighted quantum state
        fitness_sorted = np.argsort(r)
        fitness_rank = np.where(fitness_sorted == i)[0][0] / self.n_individuals
        quantum_weight = 1.0 / (1.0 + 3.0 * fitness_rank)
        
        # Base crossover probability with quantum modulation
        quantum_position = idx / self.n_individuals
        base_prob = 0.6 + 0.35 * np.sin(quantum_phases[i]) * superposition_amp
        base_prob *= quantum_weight * (0.8 + 0.2 * quantum_position)
        
        p_cr[i] = np.clip(base_prob, self.p_min, 0.85)
        
        # Dimensional entanglement with quantum interference
        entanglement_phases = np.linspace(0, 2 * np.pi, self.ndim_problem)
        
        if self.h > 0 and len(self.m_median) > 0:
            # Historical coherence modulation
            median_var = np.var(self.m_median)
            interference_pattern = median_var * np.sin(entanglement_phases + coherence_time)
            entanglement_phases += interference_pattern
        
        # Decoherence factor for dimensional coupling
        decoherence_factor = np.exp(-0.5 * quantum_position) * np.cos(coherence_time + quantum_position * np.pi)
        dim_coupling = 0.75 + 0.25 * np.cos(entanglement_phases + quantum_phases[i]) * decoherence_factor
        dim_crossover_probs = dim_coupling * p_cr[i]
        
        # Apply dimensional crossover mask
        crossover_mask = self.rng_optimization.random(self.ndim_problem) < dim_crossover_probs
        
        # Ensure minimum crossover activity
        if not np.any(crossover_mask):
            peak_dim = np.argmax(dim_crossover_probs)
            crossover_mask[peak_dim] = True
        
        # Quantum entangled partner selection
        entanglement_distance = max(1, int(2 * (1 - quantum_position) + 1))
        partner_idx = quantum_order[(idx + entanglement_distance) % self.n_individuals]
        
        # Execute crossover with entangled partner
        x_cr[i, crossover_mask] = x[partner_idx, crossover_mask]
        
        # Quantum tunneling effect for selected dimensions
        tunneling_probability = 0.25 + 0.25 * np.sin(coherence_time + quantum_position * 3 * np.pi)
        tunneling_dims = crossover_mask & (self.rng_optimization.random(self.ndim_problem) < 0.4)
        
        if np.any(tunneling_dims):
            x_cr[i, tunneling_dims] = (tunneling_probability * x_mu[i, tunneling_dims] + 
                                     (1 - tunneling_probability) * x[partner_idx, tunneling_dims])
...
def run_single(problem_index, run_index, rand_seed):
    ...
    save_individual_run(run_index, gnbg, result_path, problem_index, OptimumValue)
...
def save_individual_run(run_index, gnbg, result_path, problem_index, optimum_value):
    # 为每个run创建单独的文件
    individual_path = os.path.join(result_path, f"Problem_{problem_index}_Run_{run_index}.xlsx")

    df = pd.DataFrame({
        'Run': [run_index],
        'BestFoundResult': [abs(gnbg.BestFoundResult - optimum_value)],
...
def main():
    ...
    num_processes = 8  # Get the number of available CPU cores
    total_runs = 31
