...
def change_population(self, x=None, y=None, a=None, args=None):
    ...
    if hasattr(self, '_fitness_history') and len(self._fitness_history) > 0:
        recent_best = np.min(y)
        if len(self._fitness_history) >= 5:
            improvement_rate = (self._fitness_history[-5] - recent_best) / (self._fitness_history[-5] + 1e-10)
        else:
            improvement_rate = (self._fitness_history[0] - recent_best) / (self._fitness_history[0] + 1e-10)
        self._fitness_history.append(recent_best)
    else:
        self._fitness_history = [np.min(y)]
        improvement_rate = 0.1
    
    # Calculate convergence velocity based on population variance
    fitness_var = np.var(y)
    normalized_var = fitness_var / (np.mean(np.abs(y)) + 1e-10)
    convergence_velocity = 1.0 / (normalized_var + 1e-6)
    
    # Determine population size based on gradient and velocity
    progress = self._n_generations / max(1, self.max_function_evaluations // self.initial_pop_size)
    
    # Base size calculation using exponential decay
    decay_factor = np.exp(-2.5 * progress)
    base_size = int(self.initial_pop_size * (0.4 + 0.6 * decay_factor))
    
    # Adjust based on improvement rate
    if improvement_rate > 0.05:  # Good improvement
        size_multiplier = 1.3
    elif improvement_rate > 0.01:  # Moderate improvement
        size_multiplier = 1.0
    else:  # Poor improvement
        if convergence_velocity > 10:  # Fast convergence, reduce size
            size_multiplier = 0.7
        else:  # Slow convergence, increase exploration
            size_multiplier = 1.1
    
    new_size = max(6, min(int(base_size * size_multiplier), self.initial_pop_size * 2))
    
    # Apply population size change
    if new_size > self.n_individuals:
        # Expand population with elite-guided generation
        n_new = new_size - self.n_individuals
        new_x = np.zeros((n_new, self.ndim_problem))
        
        # Select elite individuals (top 20%)
        elite_count = max(1, len(x) // 5)
        elite_indices = np.argsort(y)[:elite_count]
        
        for i in range(n_new):
            if improvement_rate < 0.01:  # Exploration mode
                # Combine elite with archive solutions
                elite_idx = self.rng_optimization.choice(elite_indices)
                archive_idx = self.rng_optimization.integers(0, len(a))
                crossover_point = self.rng_optimization.uniform(0.2, 0.8)
                new_x[i] = crossover_point * x[elite_idx] + (1 - crossover_point) * a[archive_idx]
                # Add exploration noise
                noise_scale = 0.15 * (1 - progress * 0.5)
                new_x[i] += self.rng_optimization.normal(0, noise_scale, self.ndim_problem)
            else:  # Exploitation mode
                # Perturbation around best solutions
                elite_idx = self.rng_optimization.choice(elite_indices)
                perturbation_scale = 0.05 / (convergence_velocity ** 0.3)
                new_x[i] = x[elite_idx] + self.rng_optimization.normal(0, perturbation_scale, self.ndim_problem)
        
        x = np.vstack([x, new_x])
        y = np.hstack([y, np.full(n_new, np.inf)])
        a = np.vstack([a, new_x])
        
    elif new_size < self.n_individuals:
        # Contract population using gradient-based selection
        if improvement_rate > 0.02:  # Elite selection for good improvement
            keep_indices = np.argsort(y)[:new_size]
        else:  # Tournament selection with diversity consideration
            keep_indices = []
            ...
            remaining_pool = list(range(len(x)))
            remaining_pool.remove(best_idx)
            
            for _ in range(new_size - 1):
                if len(remaining_pool) < 3:
                    keep_indices.extend(remaining_pool)
                    break
                tournament_size = min(3, len(remaining_pool))
                tournament = self.rng_optimization.choice(remaining_pool, tournament_size, replace=False)
                winner = tournament[np.argmin(y[tournament])]
        ...
        # Maintain archive size proportional to population
        if len(a) > new_size * 2:
            archive_keep = min(new_size * 2, len(a))
            archive_indices = self.rng_optimization.choice(len(a), archive_keep, replace=False)
            a = a[archive_indices]
    
    self.n_individuals = new_size
    return x, y, a'''
...
def run_single(problem_index, run_index, rand_seed):
    ...
    program_callable = all_globals_namespace['change_population']
    de.change_population = types.MethodType(program_callable, de)
    history = de.optimize()
    print(history)
